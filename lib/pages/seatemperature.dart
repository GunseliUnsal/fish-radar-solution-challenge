...
class SeaTemperature extends StatelessWidget {
  ...
  const SeaTemperature({Key? key, required this.onComplete}) : super(key: key);
    ...
    return Scaffold(
      ...
      floatingActionButton: FloatingActionButton(
        onPressed: () async {
          ...
          bool alreadyClicked = prefs.getBool('seaTemperatureClicked') ?? false;

          if (!alreadyClicked) {
            // Call the callback with the current progress
            onComplete(0.170);
            prefs.setBool('seaTemperatureClicked', true);
